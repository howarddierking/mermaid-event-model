...
4. **For each slice**, do:
   ...
   - **Classify the slice's pattern.** Every slice is one of the four canonical Event Modeling patterns, and the classification is a **derived fact** — computed from the slice's edges, never declared by the model author. The authoritative rules live in `skills/add-slices/SKILL.md` under "Background: the four canonical patterns"; read it if a slice is ambiguous. The decision procedure:

     | Slice shape | Pattern |
     |---|---|
     | has `readModel → automation` **and** `automation → command`, any read-side event is an `externalEvent` | **Translation** |
     | has `readModel → automation` **and** `automation → command`, all read-side events are `domainEvent`s | **Automation** |
     | `event(s) → readModel`, optionally `readModel → ui`, no command | **View** |
     | `ui → command → event(s)` | **Command** |
     | `externalEvent → command → event(s)`, no view or automation | **Translation** `[abbreviated]` |
     | `domainEvent → command → event(s)`, no view or automation | **Automation** `[abbreviated]` |
     | anything else | **Unclassified** — record it as such and report it |

     The key point: Automation and Translation are structurally identical, so `externalEvent` — the only construct in the DSL that marks a system boundary — is the sole discriminator. And a slice triggered by an event is never a Command Pattern, whose trigger is a *user*.
   ...
   - **If the spec file does NOT exist:** read the template, substitute placeholders, write the file.
     ...
     - `{{SLICE_PATTERN}}` → the classified pattern, with the `[abbreviated]` tag when it applies
   ...
   - **If the spec file DOES exist:** the skill is re-runnable. Refresh the Model section in place, leaving Description and Tests untouched.
     ...
     3. Replace that section's content with the freshly-derived `**Pattern:**` line followed by a fresh ` ```mermaid ` fence containing `eventModel` and the freshly-built snippet body, ending with the closing fence. The replacement must include exactly one `## Model` heading, one `**Pattern:**` line, and one fenced block. Both the pattern and the model body are derived, so both are overwritten wholesale — a hand-edited pattern will not survive, which is intentional.
